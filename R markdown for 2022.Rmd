...
First, I obtain a count of the total conversations initiated by filtering the data for cases where `messaged_first` is true for either **me** or **her**.  Then I group by `app` and count the number of total conversations initiated for each `app`. I name this variable `conversation_initiated`.
...
```{r}
#plot of number of first dates and number of second dates
ggplot(filtered_first_date, aes(x = app, fill = X2nd_date)) + geom_bar() + 
  labs(title = "How many different people did the user go on a first date with?", x = " ", y = " ") +
...
First, I change the data type of `cost` to numeric
